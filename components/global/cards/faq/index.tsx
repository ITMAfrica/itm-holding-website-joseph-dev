...
export default function CardFaq({ question, response, about = false }: any) {
  ...
  if (about)
    return (
      <article className="w-full group bg-white flex flex-wrap items-center justify-between text-black p-5 pl-0 mb-3 border-b ">
        <div
          onClick={showUs}
          className="w-full cursor-pointer h-fit flex justify-between items-start"
        >
          <h1
            className={`text-blue_itm_aqua_marine text-xl font-bold cursor-pointer ${
  ...
  return (
    <article className="w-full group bg-white flex flex-wrap items-center justify-between text-black px-5 py-1 mb-3 rounded-lg">
      ...
      <div
        onClick={showUs}
        className={`w-fit rounded-full p-3 group-hover:text-blue_itm_aqua_marine ${
          !show ? 'text-gray_itm_bg' : 'text-blue_itm_aqua_marine'
        } text-2xl cursor-pointer`}
      >
        {!show ? <FaPlus /> : <FaMinus />}
      </div>
